...
async function ensureHost(conn: ConnRow, extra: MoonlightExtraConfig): Promise<{ hostId: number; extra: MoonlightExtraConfig }> {
  ...
  // Restore pairing material into MLW storage if we have a backup and MLW lost it
  if (hostId) {
    ...
    try {
      ...
      // Host missing from MLW — recreate below
      hostId = undefined;
    }
  }
  ...
  // If we have encrypted pairing for this connection from a previous host id, try restore
  const stored = loadEncryptedPairing(conn.id);
  if (stored?.pairInfo) {
...
// ── POST /:id/pair ───────────────────────────────────────────────
// NDJSON stream: {"pin":"1234"}\n then {"paired":true,"hostId":N}\n or {"error":"..."}\n
...
// ── POST /:id/session ────────────────────────────────────────────
// Returns stream URL for the embedded moonlight-web viewer.
